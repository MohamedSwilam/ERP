<template>
  <section>
    ...
    <b-row v-if="can('browse_buffet_order')">
      <b-col cols="12">
        <b-overlay
...
export default {
  ...
  methods: {
    ...
    browseBuffetOrders(page = 0) {
      ...
      this.$store.dispatch('buffetOrder/browse', `?paginate=${this.buffetOrders.recordsPerPage}&page=${page}&filter[search]=${this.buffetOrders.search}&customer=${this.$route.params.id}`).then(response => {
