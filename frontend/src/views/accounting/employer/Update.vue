      ...
      <b-row>
        <b-col cols="12">
          ...
          >
            ...
            >
              <validation-observer
                ref="updateEmployerForm"
                v-slot="{ handleSubmit }"
              >
                <b-form @submit.prevent="handleSubmit(updateEmployer)">
                  <b-row>
                    <!-- Name -->
                    <b-col
                      lg="6"
...
export default {
  ...
  methods: {
    viewEmployer() {
      this.employer.isCardLoading = true
      this.$store.dispatch('employers/view', this.$route.params.id).then(response => {
        this.employer.form = response.data.data
        this.employer.isCardLoading = false
      }).catch(error => {
        console.error(error)
        this.employer.isCardLoading = false
      })
    ...
    updateEmployer() {
      ...
      this.$store.dispatch('employers/update', {
        id: this.$route.params.id,
        data: this.employer.form,
      }).then(response => {
        ...
        this.$refs.updateEmployerForm.setErrors(error.response.data.errors)
        this.employer.isLoading = false
      })
    },
    reset() {
