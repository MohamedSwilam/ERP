  ...
  methods: {
    browseBuffetOrders: function browseBuffetOrders() {
      ...
      this.$store.dispatch('buffetOrder/browse', "?paginate=".concat(this.buffetOrders.recordsPerPage, "&page=").concat(page, "&filter[search]=").concat(this.buffetOrders.search)).then(function (response) {
        _this.buffetOrders.data = response.data.data;
        _this.buffetOrders.meta = response.data.meta.pagination;
        _this.buffetOrders.isLoading = false;
    ...
    deleteOrder: function deleteOrder(data) {
      ...
      this.$bvModal.msgBoxConfirm('You will not be able to retrieve this again!', {
        ...
        if (confirmed) {
          _this2.$store.dispatch('buffetOrder/delete', data.item.id).then(function (response) {
            _this2.buffetOrders.data = _this2.buffetOrders.data.filter(function (buffetOrderDetails) {
