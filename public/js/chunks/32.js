...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    browseRoles: function browseRoles() {
      ...
      this.$store.dispatch('rolesAndPermissions/browse', '').then(function (response) {
        _this.users.roles = [{
          text: 'All',
          value: ''
        }].concat(_toConsumableArray(response.data.data.map(function (role) {
          return {
            text: role.name,
            value: role.id
          };
    ...
    browseUsers: function browseUsers() {
      ...
      this.$store.dispatch('user/browse', "?paginate=".concat(this.users.recordsPerPage, "&page=").concat(page, "&filter[search]=").concat(this.users.search, "&filter[role]=").concat(this.users.role)).then(function (response) {
        _this2.users.data = response.data.data;
        _this2.users.meta = response.data.meta.pagination;
        _this2.users.isLoading = false;
      })["catch"](function (error) {
        console.error(error);
        _this2.users.isLoading = false;
      });
    ...
    deleteUser: function deleteUser(data) {
      ...
      this.$bvModal.msgBoxConfirm('You will not be able to retrieve this again!', {
        ...
        if (confirmed) {
          _this3.$store.dispatch('user/delete', data.item.id).then(function (response) {
            _this3.users.data = _this3.users.data.filter(function (user) {
